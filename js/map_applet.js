...
//var world = JSON.parse("./world-50m.json");
// var world = {};
// 
...
function removeFromUndo(command) {

  var newStack = [];
  for (var i=0; i!=undoStack.length;i++) {
    if (undoStack[i] != command) {
...
function geodesic_draw() {
  ...
  // Only one geodesic can appear at a time, so clear previous from undo stack
  removeFromUndo(clearGeodesic);
  pushUndo(clearGeodesic);
};
...
function loxodrome_draw() {
    ...
    loxodrome_ex
      ...
      .attr("d", path);
		...
		// Only one loxodrome can appear at a time, so clear previous from undo stack
    removeFromUndo(clearLoxodrome);
		pushUndo(clearLoxodrome);
};
